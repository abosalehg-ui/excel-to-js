...
[![Tests](https://img.shields.io/badge/Tests-172%2F172%20✓-2f7d4f?style=for-the-badge)](tests.html)
...
| Generator       |            20 | توليد JS، فحص عدد الوسائط، تتبع الخلايا (natural sort)، ترتيب helpers topological، حدود النطاق                                   |
...
| Excel Semantics |             6 | `MOD` بسالب، `ROUND` نصف بعيداً عن الصفر، `#DIV/0!`، `COUNTIFS` بأطوال مختلفة، `SUBSTITUTE`                                      |
| Whitespace      |             3 | فراغات بادئة قبل `=` مع بقاء مواضع الأخطاء صحيحة                                                                                 |
| Limits          |             5 | عمق التداخل، طول المُدخل، إجمالي الخلايا، حجم النطاق                                                                             |
...
| UI: CSS guard   |             5 | تصريحات المحرر والثيم الليلي والـtoast لا تعود للخلف                                                                             |
| UI: DOM         |            17 | تحويل، تظليل، تحديد الأخطاء، أزرار الأمثلة، البحث، التصفية، المسح/التراجع، النسخ، الثيم، الدلالة                                 |
| **المجموع**     | **172/172 ✓** | تشغيل في ~1.5s (منها jsdom)                                                                                                      |
...
> اختبارات `UI: DOM` تحتاج `jsdom` (تبعية تطوير). بدون `npm install` تتخطّى نفسها بصمت، فيبقى `node tests/run-node.js` صالحاً للتشغيل المباشر، ويبقى `tests.html` بلا أي أداة خارجية كما كان.
...
│   ├── suite.js            ← 145 اختباراً للمُحرّك (متصفح + Node)
│   ├── suite-ui.js         ← 27 اختباراً للواجهة (Node + jsdom)
